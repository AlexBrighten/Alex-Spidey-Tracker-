  ...
  return (
    <>
      ...
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 bg-gray-50/70 backdrop-blur-sm z-40"
        onClick={onClose}
      />
      ...
      >
        <div className="glass-card rounded-t-2xl sm:rounded-2xl border border-black/10 p-5">
          {/* Header */}
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-base font-bold text-black flex-1">Export Daily Report</h2>
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center
                         text-black/40 hover:text-black transition-all"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          {/* Report preview */}
          {hasReport ? (
            <pre className="text-xs text-black/60 font-mono leading-relaxed bg-gray-50/30 border border-black/6
                            rounded-xl p-4 overflow-y-auto max-h-64 whitespace-pre-wrap break-words mb-4">
              {report}
            </pre>
          ) : (
            <div className="flex flex-col items-center gap-2 py-8 mb-4 bg-gray-50/20 border border-black/6 rounded-xl">
              <AlertCircle className="w-8 h-8 text-orange-400/60" />
              <p className="text-black/40 text-sm text-center">
                No habit data loaded yet.<br />
                <span className="text-black/25 text-xs">Check your Firestore security rules.</span>
              </p>
            </div>
          )}
