...
const NORMAL_CATEGORIES = HABIT_CATEGORIES;
  ...
  return (
    ...
    >
      ...
      <header className="mb-6 pixel-card p-4 text-center">
        <p className="text-black/50 text-sm uppercase font-bold">{greeting()}, {user?.displayName?.split(' ')[0]} 👋</p>
        <h1 className="text-xl font-bold text-black uppercase mt-1">{formatDateDisplay(today)}</h1>
      </header>
        ...
        <>
          ...
          <div className="pixel-card p-5 mb-6">
            <div className="flex items-center gap-5">
              ...
              <div>
                <p className={`text-3xl font-black leading-none text-shadow
                  ${isAllDone ? 'text-[#16a34a]' : percentage >= 50 ? 'text-[#dc2626]' : 'text-black'}`}>
                  {percentage}%
                </p>
                <p className="text-black/50 text-sm mt-2 uppercase font-bold">{score} / {HABITS.length} DONE</p>
                <div className="mt-2">
            ...
            {isAllDone && (
              <div
                className="mt-4 flex items-center gap-3 px-4 py-3
                           bg-[#16a34a] border-[3px] border-black text-white rounded-xl"
                style={{ boxShadow: '4px 4px 0px #000' }}
              >
                <span className="text-[14px]">🎉</span>
                <div>
                  <p className="font-bold text-sm uppercase">Mission Accomplished!</p>
                  <p className="text-xs uppercase mt-1">All {HABITS.length} habits completed.</p>
                </div>
              </div>
            )}
          </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-3 h-3 border-[3px] border-black flex-shrink-0"
                  ...
                  <span className="text-sm font-bold text-black/40 ml-auto uppercase">{catDone} / {categoryHabits.length}</span>
                </div>

                {/* Category progress bar */}
                <div className="h-5 bg-gray-100 border-[3px] border-black mb-4 p-0.5 rounded-full" style={{ boxShadow: '2px 2px 0px #000' }}>
                  <div
          ...
          <button
            ...
            className="btn-ghost w-full flex items-center justify-center gap-3 mt-6 text-sm"
          >
